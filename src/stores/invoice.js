...
const DUMMY_INVOICE_DATA = [
    { 
        ...
        status: 'Lunas', 
        dp: 1200000,
        items: [{ selectedItem: { id: 5, name: 'Banner X-Stand 60x160' }, quantity: 10, total: 1200000 }]
    },
    { 
        id: 2, 
        ...
        status: 'Tertunda',
        dp: 200000,
        items: [{ selectedItem: { id: 13, name: 'Kertas A4 80gr (rim)' }, quantity: 10, total: 550000 }]
    },
...
export const useInvoiceStore = defineStore('invoice', () => {
  ...
  const nextInvoiceNumber = computed(() => {
    if (invoices.value.length === 0) return `INV-${new Date().getFullYear()}-001`;
    const lastNum = Math.max(...invoices.value.map(inv => parseInt(inv.invoiceNumber.split('-')[2])));
  ...
  async function fetchAllInvoices() {
    ...
    if (import.meta.env.VITE_MOCK_API === 'true') {
      console.log("MODE DEMO: Menggunakan data invoice dummy dari store.");
      invoices.value = DUMMY_INVOICE_DATA;
      return; // Selesai
    }
    
    try {
      console.log("MODE PRODUKSI: Menghubungi service untuk data invoice...");
      ...
      if (Array.isArray(invoicesFromApi)) {
        ...
        console.error("Data invoice dari API bukan array! Menerima:", invoicesFromApi);
        invoices.value = [];
        throw new Error("Format data invoice dari API tidak sesuai.");
      }
    } catch (error) {
      console.error("Gagal mengambil data invoice dari service:", error);
      invoices.value = []; 
      throw error;
    }
  }
